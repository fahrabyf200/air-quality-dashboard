    ...
    <div className="p-6 md:p-8 space-y-6">
          ...
          <button onClick={() => setDropdownOpen(o => !o)}
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-bold transition-all min-w-[200px] justify-between ${
              selectedUser
                ...
                : 'bg-white dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-white/20 shadow-sm dark:shadow-none'
            }`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          <div key={s.label} className="flex items-center gap-3 p-3.5 rounded-2xl border border-slate-200 dark:border-white/[0.07] bg-white dark:bg-white/[0.02] shadow-sm dark:shadow-none">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: `${s.color}15` }}>
              <s.icon size={14} style={{ color: s.color }} />
            </div>
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-600">{s.label}</p>
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-white/[0.07] bg-white dark:bg-white/[0.03] overflow-hidden shadow-sm dark:shadow-none">
        <div className="px-6 py-4 border-b border-slate-100 dark:border-white/[0.05] flex items-center justify-between flex-wrap gap-3 bg-slate-50/50 dark:bg-white/[0.01]">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 animate-pulse">Memuat...</p>
          </div>
        ) : rows.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            <Database size={28} className="text-slate-300 dark:text-slate-600" />
            <p className="text-sm font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Tidak ada data</p>
          </div>
        ) : (
          <>
            {/* Desktop Table */}
              ...
              <table className="w-full min-w-[850px]">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/[0.03]">
                  {rows.map((row, i) => {
                    ...
                    return (
                      <tr key={row.id ?? i} className={`hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors ${danger ? 'bg-red-500/[0.03]' : ''}`}>
                        ...
                        {hasUserIdCol && !selectedUser && (
                          <td className="px-5 py-3.5">
                            {row.user_name ? (
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-purple-500/30 to-blue-500/30 border border-slate-200 dark:border-white/10 flex items-center justify-center text-[10px] font-black text-slate-600 dark:text-slate-300 uppercase flex-shrink-0">
                  ...
                  <div key={row.id ?? i} className="p-4">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {[{ l: 'CO₂', v: `${row.co2?.toFixed(0)}`, over: row.co2 > T.co2 }, { l: 'NH₃', v: `${row.nh3?.toFixed(2)}`, over: row.nh3 > T.nh3 }, { l: 'VOC', v: `${(row.voc || 0).toFixed(2)}`, over: (row.voc || 0) > T.voc }, { l: 'Temp', v: `${t.toFixed(1)}°`, over: t > T.temp }, { l: 'Hum', v: `${h.toFixed(0)}%`, over: false }].map(s => (
                        <div key={s.l} className="bg-slate-50 dark:bg-white/[0.03] border border-slate-100 dark:border-white/5 p-2 rounded-xl text-center">
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t border-slate-100 dark:border-white/[0.05] flex items-center justify-between bg-slate-50/25 dark:bg-white/[0.005]">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-600">Hal {page} / {totalPages}</span>
                <div className="flex gap-2">
                  <button onClick={() => fetchData(page - 1, selectedUser?.id)} disabled={page === 1}
                    className="px-4 py-2 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white disabled:opacity-30 text-xs font-black uppercase transition-all">← Prev</button>
