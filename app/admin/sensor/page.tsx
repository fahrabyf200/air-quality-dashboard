      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          <div key={s.label} className="flex items-center gap-3 p-3.5 rounded-2xl border-2 border-slate-300 dark:border-slate-600 bg-[#FFFFFF] dark:bg-[#FFFFFF]/[0.02] shadow-[0px_4px_20px_rgba(0,0,0,0.05),0px_2px_6px_rgba(0,0,0,0.02)] dark:shadow-none">
            ...
            <div>
              <p className="text-[9px] font-semibold uppercase tracking-widest text-[#1E293B] dark:text-slate-400 dark:text-[#1E293B] dark:text-slate-400">{s.label}</p>
      ...
      <div className="rounded-2xl border-2 border-slate-300 dark:border-slate-600 bg-[#FFFFFF] dark:bg-[#FFFFFF]/[0.03] overflow-hidden shadow-[0px_4px_20px_rgba(0,0,0,0.05),0px_2px_6px_rgba(0,0,0,0.02)] dark:shadow-none">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <p className="text-sm font-semibold text-slate-400 dark:text-[#1E293B] dark:text-slate-400 uppercase tracking-widest">Tidak ada data</p>
          </div>
        ) : (
          <>
            {/* Desktop Table */}
                      ...
                      <tr key={row.id ?? i} className={`hover:bg-[#F8F9FA]/50 dark:hover:bg-[#FFFFFF]/[0.02] transition-colors ${danger ? 'bg-red-500/[0.03]' : ''}`}>
                        ...
                        {hasUserIdCol && !selectedUser && (
                          <td className="px-5 py-3.5">
                            {row.user_name ? (
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-purple-500/30 to-blue-500/30 border border-[#E2E8F0] dark:border-white/10 flex items-center justify-center text-[10px] font-semibold text-[#1E293B] dark:text-slate-400 dark:text-slate-300 uppercase flex-shrink-0">
            ...
            {totalPages > 1 && (
              <div className="px-6 py-4 border-t border-slate-100 dark:border-white/[0.05] flex items-center justify-between bg-[#F8F9FA]/25 dark:bg-[#FFFFFF]/[0.005]">
                <span className="text-[10px] font-semibold uppercase tracking-widest text-[#1E293B] dark:text-slate-400 dark:text-[#1E293B] dark:text-slate-400">Hal {page} / {totalPages}</span>
                <div className="flex gap-2">
                  <button onClick={() => fetchData(page - 1, selectedUser?.id)} disabled={page === 1}
                    className="px-4 py-2 rounded-2xl border border-[#E2E8F0] border-t-[1.5px] dark:border-white/10 bg-[#FFFFFF] dark:bg-[#FFFFFF]/5 text-[#1E293B] dark:text-slate-400 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white disabled:opacity-30 text-xs font-semibold uppercase transition-all">← Prev</button>
