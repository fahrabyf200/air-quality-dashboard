...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white dark:bg-[#0a0f1a] border border-slate-200 dark:border-white/10 p-3 rounded-xl shadow-2xl flex flex-col gap-2 relative overflow-hidden">
        ...
        return (
          <div key={p.dataKey} className="flex items-center justify-between gap-4 relative z-10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase text-slate-600 dark:text-slate-300">{cleanName}</span>
            </div>
...
export default function ReportsPage() {
  ...
  const summaryStats = [
    ...
    { label: 'Avg VOC', value: avg(vocs).toFixed(2), unit: 'PPM', color: '#ec4899', danger: avg(vocs) > T.voc, icon: Activity },
    { label: 'Max VOC', value: maxVal(vocs).toFixed(2), unit: 'PPM', color: '#f472b6', danger: maxVal(vocs) > T.voc, icon: TrendingUp },
