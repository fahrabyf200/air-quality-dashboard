...
import { 
  ...
  FileText, Activity, ShieldCheck, AlertCircle, CheckCircle2 
} from "lucide-react";
...
export default function ReportsPage() {
  ...
  const exportCSV = () => {
    ...
    const rows = data.map(i => `${i.created_at},${i.temp},${i.hum},${i.co2},${i.is_unhealthy ? 'Unhealthy' : 'Safe'}\n`);
    const blob = new Blob([...headers, ...rows], { type: 'text/csv' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `SkyWatch_Group4_Report.csv`;
    a.click();
  };
  ...
  return (
    <main className="min-h-screen p-6 md:p-12 space-y-10 bg-[#020617]">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <section className="bg-slate-900/50 border border-slate-800 p-8 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black uppercase tracking-tighter text-white">Data Archives</h1>
          ...
          >
            <Download size={20} /> Export Dataset (CSV)
          </button>
        </section>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-[2.5rem] overflow-hidden">
          <div className="p-6 border-b border-slate-800 flex justify-between items-center">
            ...
            <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">Showing latest 10 records</span>
          </div>
